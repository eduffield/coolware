resource "azurerm_managed_disk" "example_unencrypted_disk" {
  name                 = "exampleUnencryptedDisk"
...
resource "azurerm_key_vault" "example_key_vault" {
  name                        = "exampleKeyVault${random_id.unique_id.hex}"
  location                    = azurerm_resource_group.example.location
  resource_group_name         = azurerm_resource_group.example.name
  sku_name                    = "standard"
...
resource "azurerm_sql_database" "example_sql_database" {
  name                = "example-sqldb"
  resource_group_name = azurerm_resource_group.example.name
  location            = azurerm_resource_group.example.location
...
resource "azurerm_storage_account" "example_storage_account" {
  name                     = "examplestorage${random_id.unique_id.hex}"
  resource_group_name      = azurerm_resource_group.example.name
  location                 = azurerm_resource_group.example.location
...
resource "azurerm_linux_virtual_machine" "example_vm" {
  ...
  admin_password      = "AComplexPassw0rd!"
